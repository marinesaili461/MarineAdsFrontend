...
const ALL_SECTIONS = [
  ...
  { key: "permissions",   label: "Permissions",   icon: "fa-shield-halved", to: "/admin/permissions", superadminOnly: true },
];
...
const AdminLayout = () => {
  ...
  const visibleSections = ALL_SECTIONS.filter((s) => {
    ...
    if (!isSuperAdmin && hidden.includes(s.key)) return false;
    return true;
  });
  ...
  const linkClass = ({ isActive }) =>
    `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all ${
      isActive ? "bg-orange-500 text-white shadow" : "text-gray-600 hover:bg-orange-50 hover:text-orange-500"
    }`;
    ...
    <div className="flex h-screen bg-gray-100 overflow-hidden">
      ...
      `}>
        ...
        <div className="bg-orange-400 flex items-center gap-3 px-4 py-4">
          <img src={logo} alt="logo" style={{ width: "40px", height: "40px", borderRadius: "50%", objectFit: "cover" }} />
          <div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-orange-400 h-14 flex items-center justify-between px-4 shadow-md shrink-0">
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className="lg:hidden text-white text-xl">
            <i className="fas fa-bars"></i>
          </button>
          <h1 className="text-white font-bold text-base">Admin Dashboard</h1>
          <div className="flex items-center gap-2">
            {isSuperAdmin && (
              <span className="bg-white text-orange-500 text-xs font-bold px-2 py-1 rounded-full">Super Admin</span>
            )}
